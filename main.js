...
// Main spotlight with reduced intensity
const mainSpotLight = new THREE.SpotLight(0xffffff, 300);
mainSpotLight.position.set(5, 12, 5);
...
// Add fill light for subtle details (reduced intensity)
const fillLight = new THREE.SpotLight(0xffffff,40);
...
const rightRimLight = new THREE.SpotLight(0xffffff, 100);
rightRimLight.position.set(8, 6, 0);
...
const leftRimLight = new THREE.SpotLight(0xffffff, 100);
leftRimLight.position.set(-8, 6, 0);
...
function updateCarColor(color) {
    if (carMaterial) {
        carMaterial.color.setHex(color);
        carMaterial.metalness = 0.4;
        carMaterial.roughness = 0.3;
        carMaterial.envMapIntensity = 2.0;
        carMaterial.emissive.setHex(color);
...
const loader = new GLTFLoader(loadingManager).setPath('public/audi_rs5/');
loader.load('scene.gltf', (gltf) => {
    ...
    mesh.traverse((child) => {
        if (child.isMesh) {
            ...
            // Store the main body material reference
            if (child.name.toLowerCase().includes('body') || 
                child.name.toLowerCase().includes('car') ||
                child.name.toLowerCase().includes('karosserie')) {
                carMaterial = child.material;
                console.log('Found car material:', child.name);
            }
        }
    });
